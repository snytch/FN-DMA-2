...
//???
#define 	cmd_connect			0xaf3c2828 //ok????
#define     cmd_mouse_move		0xaede7345 //ok????
#define		cmd_mouse_left		0x9823AE8D //ok??????
#define		cmd_mouse_middle	0x97a3AE8D //ok??????
#define		cmd_mouse_right		0x238d8212 //ok??????
#define		cmd_mouse_wheel		0xffeead38 //ok??????
#define     cmd_mouse_automove	0xaede7346 //ok????????????
#define     cmd_keyboard_all    0x123c2c2f //ok????????
#define		cmd_reboot			0xaa8855aa //ok????
#define     cmd_bazerMove       0xa238455a //ok???????
#define     cmd_monitor         0x27388020 //ok????????????
#define     cmd_debug           0x27382021 //ok??????
#define     cmd_mask_mouse      0x23234343 //ok ??????
#define     cmd_unmask_all      0x23344343 //ok ????????
#define     cmd_setconfig       0x1d3d3323 //ok ??IP????
#define     cmd_setvidpid       0xffed3232 //ok ??device??vidpid
#define     cmd_showpic         0x12334883 //????


extern SOCKET sockClientfd; //socket????
typedef struct
{
	unsigned int  mac;			//???mac??????
	unsigned int  rand;			//???
...
enum
{
	err_creat_socket = -9000,	//??socket??
	err_net_version,		//socket????
	err_net_tx,		//socket????
	err_net_rx_timeout,		//socket????
	err_net_cmd,			//????
	err_net_pts,			//?????
	success = 0,				//????
	usb_dev_tx_timeout,		//USB devic????
};
...
int kmNet_monitor_mouse_side1();		//??????1??
int kmNet_monitor_mouse_side2();		//??????2?? 
...
int kmNet_setconfig(char* ip, unsigned short port);		  //????IP??
int kmNet_setvidpid(unsigned short vid, unsigned short pid);//????VIDPID ??????????????????
int kmNet_debug(short port, char enable);				  //????
int kmNet_lcd_color(unsigned short rgb565);				  //???LCD?????????? ???????
int kmNet_lcd_picture_bottom(unsigned char* buff_128_80); //??????128x80??
int kmNet_lcd_picture(unsigned char* buff_128_160);		  //????128x160??


//?????????????? ????device??????
int kmNet_DevDescriptor(int wrflag, char* buff, int length);//???????
int kmNet_CfgDescriptor(int wrflag, char* buff, int length);//?????????
int kmNet_EndPointDescriptor(int wrflag, int ep, char* buff, int length);//???????


//???????
#if 0
int kvm_setvideo(int width, int height, int fps);//???????????
int kvm_getframe(Mat* frame);					 //??????????
int kvm_load_yolo_module(char* path);			 //??yolo????
int kvm_run_yolo(Mat* frame, int cpu_gpu);		 //????
#endif
