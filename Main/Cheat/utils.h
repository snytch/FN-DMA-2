...
inline Vector3 w2s(Vector3 WorldLocation)
{
	...
	return Vector3((settings::window::Width / 2.0f) + vTransformed.x * (((settings::window::Width / 2.0f) / tanf(mainCamera.FieldOfView * (float)M_PI / 360.f))) / vTransformed.z, (settings::window::Height / 2.0f) - vTransformed.y * (((settings::window::Width / 2.0f) / tanf(mainCamera.FieldOfView * (float)M_PI / 360.f))) / vTransformed.z, 0);
}